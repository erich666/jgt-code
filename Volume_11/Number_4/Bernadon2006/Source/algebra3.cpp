...
double& vec2::operator [] (int i) {
    if (i < VX || i > VY)
	MSG_ERROR("vec2 [] operator: illegal access; index = " << i << '\n')
    return n[i];
}
...
double& vec3::operator [] (int i) {
    if (i < VX || i > VZ)
	MSG_ERROR("vec3 [] operator: illegal access; index = " << i << '\n')
    return n[i];
}
...
double& vec4::operator [] (int i) {
    if (i < VX || i > VW)
	MSG_ERROR("vec4 [] operator: illegal access; index = " << i << '\n')
    return n[i];
}
...
vec3& mat3::operator [] (int i) {
    if (i < VX || i > VZ)
	MSG_ERROR("mat3 [] operator: illegal access; index = " << i << '\n')
    return v[i];
}
...
mat3 mat3::inverse()	    // Gauss-Jordan elimination with partial pivoting
    ...
    if (a.v[j].n[j]==0.)
		return b;
	MSG_ERROR("mat3::inverse: singular matrix; can't invert\n")
    b.v[j] /= a.v[j].n[j];
    a.v[j] /= a.v[j].n[j];
...
vec4& mat4::operator [] (int i) {
    if (i < VX || i > VW)
	MSG_ERROR("mat4 [] operator: illegal access; index = " << i << '\n')
    return v[i];
}
...
mat4 mat4::inverse()	    // Gauss-Jordan elimination with partial pivoting
{
    ...
    if (a.v[j].n[j]==0.)
		return mat4(0.0);
	//MSG_ERROR("mat4::inverse: singular matrix; can't invert\n")
    b.v[j] /= a.v[j].n[j];
    a.v[j] /= a.v[j].n[j];
