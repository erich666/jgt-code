 ...
 * rthalton.c
 * 
 * This code is written to test the effect of sampling with Halton
 * point set in ray tracing.
 * For simplicity, the average number of sample per pixel is defined by
 * macros AVE_SAMPLE_NUM. The basis are P1 and P2.
...
void raytrace(int argc, char **argv)
{
  ...
  Pixel acc, tmp;
  Float p, ip, u, v, *minptr;
  int k, kk, pos, n, maxdim, mindim, a;
