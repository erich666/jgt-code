 ...
 * rtjitter.c
 * 
 * This code is written to test the effect of jitter sampling in 
 * ray tracing.
...
/* Implementation */
void jitter(int cellx, int celly, Float *sample)
{
  ...
  for (i=0, startx=0.0, count=0 ; i<cellx ; i++, startx+=intervalx)
    ...
    {
      /* gen sample */
      sample[count<<1] = startx + intervalx*random()/(double)MAXLONG;
      sample[(count<<1)+1] = starty + intervaly*random()/(double)MAXLONG;
    }
  ...
  if (counter%5000==0)
  {
    ...
    if ((fptr=fopen("jitter.ps","wt"))==NULL)
    {
      fprintf(stderr, "Cannot open jitter.ps\n");
      exit(1);
    }
    fprintf(fptr, "%%!\n100 100 translate\ngsave\n");
