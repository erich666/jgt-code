...
// 
//    DO NOT REUSE THIS FILE!  IT'S BEEN MODIFIED FOR THIS PROGRAM!
//
// DO NOT -- DO NOT -- DO NOT -- DO NOT -- DO NOT -- DO NOT -- DO NOT
...
GLvoid glmDrawForRefraction( GLMmodel* model, double *nDist, float *curvatureData )
{
  static GLuint i;
  static GLMgroup* group;
  static GLMtriangle* triangle;
  ...
  while (group) 
  {
    ...
    for (i = 0; i < group->numtriangles; i++) 
	{
		...
		if (curvatureData)
			glMultiTexCoord4f( GL_TEXTURE7, curvatureData[ 4 * triangle->vindices[0] + 0],
				                            curvatureData[ 4 * triangle->vindices[0] + 1],
											curvatureData[ 4 * triangle->vindices[0] + 2],
											curvatureData[ 4 * triangle->vindices[0] + 3] );
		glVertexAttrib4dARB( 2, model->normals[ 3 * triangle->nindices[0] + 0 ],
								model->normals[ 3 * triangle->nindices[0] + 1 ], 
								model->normals[ 3 * triangle->nindices[0] + 2 ],
								nDist[triangle->vindices[0]]  );
		glVertex3fv(&model->vertices[3 * triangle->vindices[0]]);


		if (curvatureData)
			glMultiTexCoord4f( GL_TEXTURE7, curvatureData[ 4 * triangle->vindices[1] + 0],
				                            curvatureData[ 4 * triangle->vindices[1] + 1],
											curvatureData[ 4 * triangle->vindices[1] + 2],
											curvatureData[ 4 * triangle->vindices[1] + 3] );
		glVertexAttrib4dARB( 2, model->normals[ 3 * triangle->nindices[1] + 0 ],
								model->normals[ 3 * triangle->nindices[1] + 1 ], 
								model->normals[ 3 * triangle->nindices[1] + 2 ],
								nDist[triangle->vindices[1]]  );
		glVertex3fv(&model->vertices[3 * triangle->vindices[1]]);


		if (curvatureData)
			glMultiTexCoord4f( GL_TEXTURE7, curvatureData[ 4 * triangle->vindices[2] + 0],
				                            curvatureData[ 4 * triangle->vindices[2] + 1],
											curvatureData[ 4 * triangle->vindices[2] + 2],
											curvatureData[ 4 * triangle->vindices[2] + 3] );
		glVertexAttrib4dARB( 2, model->normals[ 3 * triangle->nindices[2] + 0 ],
								model->normals[ 3 * triangle->nindices[2] + 1 ], 
								model->normals[ 3 * triangle->nindices[2] + 2 ],
								nDist[triangle->vindices[2]]  );
		glVertex3fv(&model->vertices[3 * triangle->vindices[2]]);
...
GLvoid glmDrawNormalsForRefraction( GLMmodel* model, double *nDist, float *curvatureData )
{
  static GLuint i;
  static GLMgroup* group;
  static GLMtriangle* triangle;
    ...
    for (i = 0; i < group->numtriangles; i++) 
	{
		...
		if (curvatureData)
			glMultiTexCoord4f( GL_TEXTURE7, curvatureData[ 4 * triangle->vindices[0] + 0],
											...
											curvatureData[ 4 * triangle->vindices[0] + 2],
											curvatureData[ 4 * triangle->vindices[0] + 3] );
		glColor4d( (model->normals[ 3 * triangle->nindices[0] + 0 ] + 1) / 2,
				   (model->normals[ 3 * triangle->nindices[0] + 1 ] + 1) / 2, 
				   (model->normals[ 3 * triangle->nindices[0] + 2 ] + 1) / 2,
		           nDist[triangle->vindices[0]] / (2*sqrt(3.0)) );
		glVertex3fv(&model->vertices[3 * triangle->vindices[0]]);
		...
		if (curvatureData)
			glMultiTexCoord4f( GL_TEXTURE7, curvatureData[ 4 * triangle->vindices[1] + 0],
											...
											curvatureData[ 4 * triangle->vindices[1] + 2],
											curvatureData[ 4 * triangle->vindices[1] + 3] );
		glColor4d( (model->normals[ 3 * triangle->nindices[1] + 0 ] + 1) / 2,
				   (model->normals[ 3 * triangle->nindices[1] + 1 ] + 1) / 2, 
				   (model->normals[ 3 * triangle->nindices[1] + 2 ] + 1) / 2,
				   nDist[triangle->vindices[1]] / (2*sqrt(3.0)) );
		glVertex3fv(&model->vertices[3 * triangle->vindices[1]]);
		...
		if (curvatureData)
			glMultiTexCoord4f( GL_TEXTURE7, curvatureData[ 4 * triangle->vindices[2] + 0],
											...
											curvatureData[ 4 * triangle->vindices[2] + 2],
											curvatureData[ 4 * triangle->vindices[2] + 3] );
		glColor4d( (model->normals[ 3 * triangle->nindices[2] + 0 ] + 1) / 2,
				   (model->normals[ 3 * triangle->nindices[2] + 1 ] + 1) / 2, 
				   (model->normals[ 3 * triangle->nindices[2] + 2 ] + 1) / 2,
				   nDist[triangle->vindices[2]] / (2*sqrt(3.0)) );
		glVertex3fv(&model->vertices[3 * triangle->vindices[2]]);
...
/* glmBoundingSphereRadius: finds the origin and bounding sphere of
 *    the model.  The radius is optimal given the center, however the
 *    center of the bounding sphere returned may not be the optimal
 *    center!  (All this does is take the bounding box's center, and
 *    compute the max distance to any vertex)
 */
GLfloat
glmBoundingSphereRadius(GLMmodel* model, GLfloat *center)
{
  GLuint  i;
  GLfloat maxRadius = 0;
  ...
  for (i = 1; i <= model->numvertices; i++) 
  {
	  tmpR = (model->vertices[3 * i + 0]-center[0]) * (model->vertices[3 * i + 0]-center[0]) +
			(model->vertices[3 * i + 1]-center[1]) * (model->vertices[3 * i + 1]-center[1]) +
			(model->vertices[3 * i + 2]-center[2]) * (model->vertices[3 * i + 2]-center[2]) ;
