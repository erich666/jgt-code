/* $Id: gl2ps.c,v 1.4 2006/09/05 11:31:43 CVS Exp $ */
/*
...
static void gl2psPrintIPEHeader(void)
{
  GLint index ;
  GLfloat rgba[4] ;
  time_t now ;
  ...
  fprintf( gl2ps->stream,
           "<ipe>\n<info title=\"%s\" author=\"%s\"/>\n"
           "<page mediabox=\"%d %d %d %d\">\n",
           gl2ps->title, gl2ps->producer,
  ...
  if( gl2ps->options & GL2PS_DRAW_BACKGROUND )
  {
    ...
    fprintf( gl2ps->stream,
             "<path fill=\"%g %g %g\"> %d %d m %d %d l %d %d l %d %d l h </path>\n",
             rgba[0], rgba[1], rgba[2],
             gl2ps->viewport[0], gl2ps->viewport[1],
             gl2ps->viewport[2], gl2ps->viewport[1],
             gl2ps->viewport[2], gl2ps->viewport[3],
             gl2ps->viewport[0], gl2ps->viewport[3] ) ;
  }
}

static void gl2psPrintIPEPrimitive(void *data)
{
  GL2PSprimitive *prim;
  ...
  case GL2PS_TEXT :
    ...
    gl2psPrintf("<text size=\"%d\" stroke=\"%g %g %g\" pos=\"%g %g\">",
                prim->data.text->fontsize,
                prim->verts[0].rgba[0], prim->verts[0].rgba[1], prim->verts[0].rgba[2],
                prim->verts[0].xyz[0], prim->verts[0].xyz[1] ) ;
    gl2psPrintf("\\fontsize{%d}{0}\\selectfont{\\makebox(0,0)",
            prim->data.text->fontsize);
    switch(prim->data.text->alignment){
    case GL2PS_TEXT_CL:
      gl2psPrintf("[l]{");
    ...
    if(prim->data.text->angle)
      gl2psPrintf("\\rotatebox{%g}{", prim->data.text->angle);
  ...
  case GL2PS_POINT :
    gl2psSetLastColor( prim->verts[0].rgba ) ;
    gl2psPrintf(" <mark stroke=\"%g %g %g\" shape=\"2\" pos=\"%g %g\" size=\"%g\"/>\n",
             gl2ps->lastrgba[0], gl2ps->lastrgba[1], gl2ps->lastrgba[2],
             prim->verts[0].xyz[0], prim->verts[0].xyz[1], 0.5*prim->width ) ;
    break;
  case GL2PS_LINE :
    if( gl2ps->lastlinewidth != prim->width )
    ...
    gl2psPrintf("<path stroke=\"%g %g %g\" dash=\"%s\" pen=\"%g\"> %g %g m %g %g l </path>\n",
             gl2ps->lastrgba[0], gl2ps->lastrgba[1], gl2ps->lastrgba[2],
             prim->pattern? "dashed" : "solid", gl2ps->lastlinewidth,
             prim->verts[1].xyz[0], prim->verts[1].xyz[1],
             prim->verts[0].xyz[0], prim->verts[0].xyz[1] ) ;
    break ;
  case GL2PS_TRIANGLE :
    gl2psSetLastColor( prim->verts[0].rgba ) ;
    gl2psPrintf("<path fill=\"%g %g %g\"> %g %g m %g %g l %g %g l h </path>\n",
             gl2ps->lastrgba[0], gl2ps->lastrgba[1], gl2ps->lastrgba[2],
             prim->verts[2].xyz[0], prim->verts[2].xyz[1],
             prim->verts[1].xyz[0], prim->verts[1].xyz[1],
             prim->verts[0].xyz[0], prim->verts[0].xyz[1] ) ;
...
static void gl2psPrintIPEFooter(void)
{
  gl2psPrintf("</page>\n</ipe>\n" ) ;
}

static void gl2psPrintIPEBeginViewport(GLint viewport[4])
{
  GLint index ;
  GLfloat rgba[4] ;
  int x = viewport[0], y = viewport[1], w = viewport[2], h = viewport[3] ;
...
static GLint gl2psPrintIPEEndViewport(void)
{
  GLint res ;
...
static void gl2psPrintRAWPrimitive(void *data)
{
  GL2PSprimitive *prim;
  ...
  case GL2PS_LINE :
    break ;
  case GL2PS_TRIANGLE :
    gl2psPrintf("%g %g %g %g %g %g %g %g %g\n",
                prim->verts[2].xyz[0], prim->verts[2].xyz[1], prim->verts[2].xyz[2],
                prim->verts[1].xyz[0], prim->verts[1].xyz[1], prim->verts[1].xyz[2],
                prim->verts[0].xyz[0], prim->verts[0].xyz[1], prim->verts[0].xyz[2] ) ;
    break;
  case GL2PS_QUADRANGLE :
    gl2psMsg(GL2PS_WARNING, "There should not be any quad left to print");
    break;
...
static GLint gl2psPrintPrimitives(void)
{
  ...
  if(used > 0){
    if(gl2ps->format == GL2PS_PS  ||
       ...
       gl2ps->format == GL2PS_IPE ||
       gl2ps->format == GL2PS_RAW ){
  ...
  case GL2PS_PDF :
    ...
    break;
  case GL2PS_IPE :
    pprim = gl2psPrintIPEPrimitive;
    break;
  case GL2PS_RAW :
    pprim = gl2psPrintRAWPrimitive;
    break;
  }
...
GLint gl2psBeginViewport(GLint viewport[4])
{
  ...
  case GL2PS_PDF :
    ...
    break;
  case GL2PS_IPE :
    gl2psPrintIPEBeginViewport(viewport);
    break;
  default :
    break;
  }
